...
export default {
  ...
  computed: {
    groupedTests() {
      const groups = {};
      this.testGroups.forEach(test => {
        if (!groups[test.testType]) {
          groups[test.testType] = [];
        }
        groups[test.testType].push(test);
      });
      return groups;
    },
    latestTestDate() {
  ...
  methods: {
    ...
    async fetchTestRecords() {
      ...
      try {
        ...
        const ad8Url = `${apiBaseUrl}/user/ad8_responses`;
        const psqiUrl = `${apiBaseUrl}/user/psqi_responses`;
    ...
    exportTest(test) {
      ...
      test.responses.forEach(response => {
        content += `问题: ${response.questionText || response.question_text}\n`;
        content += `回答: ${response.answer || response.answer_text}\n\n`;
      });
    ...
    viewDetails(group) {
      ...
      group.responses.forEach(response => {
        ...
        question.textContent = response.questionText || response.question_text || `问题 ${response.question_id}`;
        qDiv.appendChild(question);

        const answer = document.createElement('p');
        answer.textContent = `回答: ${response.answer || response.answer_text || '无'}`;
