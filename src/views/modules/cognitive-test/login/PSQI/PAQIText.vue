...
export default {
  ...
  setup() {
     ...
     * 获取PSQI测试问题列表
     * @async
     * @description 从服务器获取PSQI测试问题及其子问题
     * @throws {Error} 网络请求失败或服务器返回错误时抛出
     */
    const fetchQuestions = async () => {
      try {
        loading.value = true;
        const response = await fetch('http://localhost:8080/api/psqi/questions');
    ...
    const submitAnswers = async () => {
      try {
        ...
        const response = await fetch('http://localhost:8080/api/psqi/responses', {
          method: 'POST',
          headers: {
     ...
     * @param {string} choice - 选择的选项值('1'或'0')
     */
